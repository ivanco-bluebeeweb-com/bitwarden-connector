...
(disconnect per Bitwarden organization) for Bitwarden Connector. Same
...
def _connection_row(c: dict) -> ui.UINode:
    ...
    return ui.Stack(direction="v", gap=1, align="start", children=[
        ...
        ui.Button(
            ...
            on_click=ui.Call("disconnect_bitwarden", {"connection_id": c.get("id")}),
        ),
    ])
...
def _connections_section(connections: list[dict]) -> ui.UINode:
    if not connections:
        return ui.Stack(direction="v", gap=1, children=[
            ...
            ui.Text("No Bitwarden organizations connected yet.", variant="caption"),
        ])
...
@ext.panel("bitwarden_settings", slot="center")
async def bitwarden_settings(ctx, **kwargs) -> object:
    connections = await h._load_connections(ctx)
    return ui.Stack(direction="v", gap=3, children=[
        ui.Text("Bitwarden -- App settings", variant="heading"),
        ui.Divider(),
